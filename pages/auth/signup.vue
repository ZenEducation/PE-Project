      ...
      <CardBox :class="cardClass" is-form @submit="createAccount">
        <div v-if="!verificationCodeMsg" class="-mb-6">
          ...
          <FormField label="Username" help="Please enter your username">
            <FormControl v-model.trim="form.username" :icon="mdiAccount" name="username" autocomplete="username" />
          </FormField>

          <FormField label="Family name" help="Please enter your family name">
            <FormControl v-model.trim="form.familyName" :icon="mdiAccount" name="familyName" autocomplete="familyName" />
          </FormField>
          ...
          <BaseButtons class="flex items-center justify-center">
            <BaseButton color="info" label="Create account" @click="createAccount" />
          </BaseButtons>
        ...
        <div v-else class="-mb-6">
          ...
          <BaseButtons class="flex items-center justify-center">
            ...
            <BaseButton color="info" label="Verify account" @click="verifyAccount" />
          </BaseButtons>
        </div>

        <template #footer>
          <BaseButtons v-if="!verificationCodeMsg" class="flex items-center justify-center mt-0">
            <BaseButton color="info" label="Google" @click="() =>
                Auth.federatedSignIn({
                  provider: CognitoHostedUIIdentityProvider.Google,
                })
              ">
            </BaseButton>
            <BaseButton color="info" label="Facebook" @click="() =>
                Auth.federatedSignIn({
          ...
          <BaseButtons v-else class="flex items-center justify-center mt-0">
            <BaseButton color="info" label="Sign up with different email" @click="resetState" />
          </BaseButtons>
        </template>
      </CardBox>
    </SectionFullScreen>
